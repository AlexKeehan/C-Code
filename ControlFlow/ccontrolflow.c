 ...
 */
    int length = sizeof(a);
    int x = 0; // largest value stored here
    int y = 0; // smallest value stored here
    for (int i = 0; i < length-1; i++) { //  for loop to iterate through a
	    if (a[i] > x) {
		    x = a[i]; // sets the largest value to x
		}
	    if (a[i] < y) {
		    y = a[i];
	    } // sets the smallest value to y
	    printf ("%i", x); // prints largest value
	    printf ("%i", y); // prints smallest value
	}
...
/* 2a.
 ...
 // the loop would iterate through the "C programming is fun!" string until it finds the 0 at the end of the string and then it will terminate
 */
    int c1 = 0;
    while (ca[c1] != 0)
       c1++;
...
/* 2b.
 ...
 // this would do the same thing as the while loop 1
 */
    int c2 = 0;
    char *cap = ca;
    while (*cap++ !=0)
...
/* 3b.
 ...
 Explain For Loop 2.
 // this would do the same thing as for loop 1
 */
    c2 = 0;
