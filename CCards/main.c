...
int main(int argc, char **argv) {
    ...
    deal(shuffled, &players[0].hand, &players[1].hand,1);
    printf("%s Hand:\n", players[0].name);
    printCards(players[0].hand.cards, 0, 1);
    printf("%s Hand:\n", players[1].name);
    printCards(players[1].hand.cards, 0, 1);
    ...
    for (int i = 0; i < 5; i++) {
	    Card *card[2];
	    card[0] = players[0].hand.cards[i];
	    card[1] = players[1].hand.cards[i];
